      ...
      <View style={styles.checkBoxItemLayout}>
        <View style={styles.checkBoxAndLabelLayout}>
          <CheckBoxElement
            ...
            size={20}
            textComponent={
              <View style={styles.checkBoxLabelLayout}>
                <Text style={styles.checkBoxLabelCommon}>
                  {item.title}{' '}
                  <Text style={styles.checkBoxTagNumber}> #{item.tag}</Text>
                </Text>
              </View>
            }
          />
...
export default function TabPanel({defaultIndex, headerItems, hasCheckBox}) {
        ...
        <View
          ...
          {headerItems.map((item, index) => (
            <View
              key={index}
              style={[
                item.index !== headerItems.length &&
                  styles.headerItemBorderRight,
                ...
                <Text
                  style={[
                    ...
                    focusIndex === item.index &&
                      styles.headerItemFocusLabelStyle,
                  ]}>
                  {item.label}
                </Text>
                <Text
    ...
    <View
      ...
      {!hasCheckBox && headerItems[focusIndex - 1].content?.length === 0 && (
        <View style={styles.banner}>
          <Image source={BannerEmpty} style={styles.bannerImage} />
          <Text style={styles.bannerText}>{EMPTY}</Text>
        </View>
      )}
      {/* 
            {
      ...
      {hasCheckBox && headerItems[focusIndex - 1].content?.length !== 0 && (
        <ScrollView style={styles.listTask}>
          {DATA.map((item, index) => {
            return <RenderTaskWork key={index} item={item} />;
          })}
        </ScrollView>
      )}
    </View>
